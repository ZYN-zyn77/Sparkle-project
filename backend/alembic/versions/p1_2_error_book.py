...
def upgrade():
    ...
    if _should_rebuild_error_records(inspector):
        if _table_exists(inspector, "error_records"):
            # Pre-data阶段允许破坏性重建；仅在检测到旧结构时执行，避免误删当前结构。
    ...
    if _table_exists(sa.inspect(bind), "error_records"):
        op.execute(
            ...
            "ON error_records (user_id, subject)"
        )
        op.execute(
            "CREATE INDEX IF NOT EXISTS idx_errors_next_review "
...
def downgrade():
    op.execute("DROP INDEX IF EXISTS idx_ekl_knowledge")
    op.execute("ALTER TABLE IF EXISTS public.error_knowledge_links DROP CONSTRAINT IF EXISTS uq_error_knowledge")
    op.execute("DROP TABLE IF EXISTS public.error_knowledge_links")
    op.execute("DROP INDEX IF EXISTS idx_reviews_user_time")
    op.execute("DROP INDEX IF EXISTS idx_reviews_error")
    op.execute("DROP TABLE IF EXISTS public.error_reviews")
    op.execute("DROP INDEX IF EXISTS idx_analyses_error")
    op.execute("DROP TABLE IF EXISTS public.error_analyses")
    op.execute("DROP INDEX IF EXISTS idx_errors_question_fts")
    op.execute("DROP INDEX IF EXISTS idx_errors_next_review")
    op.execute("DROP INDEX IF EXISTS idx_errors_user_subject")
    op.execute("DROP TABLE IF EXISTS public.error_records")
