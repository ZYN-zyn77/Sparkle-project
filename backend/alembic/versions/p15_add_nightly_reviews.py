...
def upgrade():
    ...
    if not table_exists(inspector, "nightly_reviews"):
        op.create_table(
            "nightly_reviews",
    ...
    if not index_exists(inspector, "nightly_reviews", "idx_nightly_reviews_user"):
        op.create_index("idx_nightly_reviews_user", "nightly_reviews", ["user_id"], unique=False)
    if not index_exists(inspector, "nightly_reviews", "idx_nightly_reviews_date"):
        op.create_index("idx_nightly_reviews_date", "nightly_reviews", ["review_date"], unique=False)


def downgrade():
    inspector = get_inspector()
    if table_exists(inspector, "nightly_reviews"):
        if index_exists(inspector, "nightly_reviews", "idx_nightly_reviews_date"):
            op.drop_index("idx_nightly_reviews_date", table_name="nightly_reviews")
        if index_exists(inspector, "nightly_reviews", "idx_nightly_reviews_user"):
