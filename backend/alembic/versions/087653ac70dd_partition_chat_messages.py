...
def upgrade():
    ...
    op.execute("ALTER TABLE chat_messages RENAME TO chat_messages_old")
    ...
    op.execute("""
    CREATE TABLE chat_messages_legacy PARTITION OF chat_messages
    ...
    op.execute("""
    CREATE TABLE chat_messages_default PARTITION OF chat_messages
        DEFAULT;
    """)

    # 4. Data Migration
    op.execute("INSERT INTO chat_messages SELECT * FROM chat_messages_old")
    ...
    op.execute("""
    CREATE UNIQUE INDEX chat_messages_message_id_created_at_key 
...
def downgrade():
    ...
    # 2. Restore old table
    op.execute("ALTER TABLE chat_messages_old RENAME TO chat_messages")
