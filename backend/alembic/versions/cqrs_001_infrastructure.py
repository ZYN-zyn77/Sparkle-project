...
def upgrade():
    ...
    if not table_exists(inspector, "event_outbox"):
    ...
    if not index_exists(inspector, "event_outbox", "idx_outbox_aggregate"):
    ...
    if not table_exists(inspector, "event_store"):
    ...
    if not index_exists(inspector, "event_store", "idx_event_store_aggregate"):
        op.create_index('idx_event_store_aggregate', 'event_store',
                        ['aggregate_type', 'aggregate_id', 'sequence_number'], unique=False)
    if not index_exists(inspector, "event_store", "idx_event_store_type"):
    ...
    if not table_exists(inspector, "processed_events"):
    ...
    if not index_exists(inspector, "processed_events", "idx_processed_events_cleanup"):
        op.create_index('idx_processed_events_cleanup', 'processed_events',
                        ['processed_at'], unique=False)
    if not index_exists(inspector, "processed_events", "idx_processed_events_group"):
    ...
    if not table_exists(inspector, "projection_metadata"):
    ...
    if not table_exists(inspector, "projection_snapshots"):
    ...
    if not index_exists(inspector, "projection_snapshots", "idx_snapshots_projection"):
    ...
    if table_exists(inspector, "projection_metadata"):
        op.execute("""
...
def downgrade():
    inspector = get_inspector()
    if table_exists(inspector, "projection_snapshots"):
        op.drop_table('projection_snapshots')
    if table_exists(inspector, "projection_metadata"):
        op.drop_table('projection_metadata')
    if table_exists(inspector, "processed_events"):
        op.drop_table('processed_events')
    if table_exists(inspector, "event_store"):
        op.drop_table('event_store')
    op.execute('DROP INDEX IF EXISTS idx_outbox_unpublished')
    if table_exists(inspector, "event_outbox"):
