...
def upgrade() -> None:
    ...
    if _should_rebuild_error_records(inspector):
        if _table_exists(inspector, "error_records"):
            # Destructive reset is acceptable in this pre-data stage; guard to avoid
            # dropping valid current schema or unrelated tables.
    ...
    if _table_exists(sa.inspect(bind), "error_records"):
        ...
        op.execute(
            "CREATE INDEX IF NOT EXISTS idx_errors_subject "
...
def downgrade() -> None:
    ...
    op.execute("DROP INDEX IF EXISTS idx_errors_subject")
