...
class TaskNotifier extends StateNotifier<TaskListState> {
  ...
  Future<void> loadTasks({TaskFilter? filter}) async {
    await _runWithErrorHandling(() async {
      final paginatedResponse = await _taskRepository.getTasks(filters: {}); // Add filter logic later
      state = state.copyWith(isLoading: false, tasks: paginatedResponse.items, currentFilter: filter);
    });
  }
  ...
  Future<void> updateTask(String id, TaskUpdate taskUpdate, {bool refresh = true}) async {
    await _runWithErrorHandling(() async {
  ...
  void _updateTaskInState(TaskModel task) {
     state = state.copyWith(
        ...
        todayTasks: state.todayTasks.map((t) => t.id == task.id ? task : t).toList(),
      );
  }
  ...
  void _updateTask(String taskId, TaskModel Function(TaskModel) updater) {
     state = state.copyWith(
        ...
        todayTasks: state.todayTasks.map((t) => t.id == taskId ? updater(t) : t).toList(),
      );
  }
}
