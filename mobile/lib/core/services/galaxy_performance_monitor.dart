...
class GalaxyPerformanceMonitor {
  ...
  void _onFrame(Duration timestamp) {
    ...
    _frameTimings.add(FrameTimingData(
      timestamp: now,
  ...
  void _checkPerformanceIssues(Duration frameDuration) {
    ...
    if (frameDuration.inMilliseconds > thresholds.maxFrameDurationMs) {
      _emitEvent(PerformanceEvent(
        ...
        message: 'Frame took ${frameDuration.inMilliseconds}ms (>${thresholds.maxFrameDurationMs}ms)',
    ...
    if (_frameTimings.length >= 3) {
      ...
      if (allSlow) {
        _emitEvent(const PerformanceEvent(
  ...
  void _emitEvent(PerformanceEvent event) {
    debugPrint('GalaxyPerformanceMonitor: ${event.severity.name} - ${event.message}');
  ...
  void _generateReport() {
    ...
    debugPrint('  Frame Time: ${report.averageFrameTimeMs.toStringAsFixed(2)}ms');
    debugPrint('  Jank Rate: ${(report.jankRate * 100).toStringAsFixed(1)}%');

    for (final metric in report.metrics.values) {
      debugPrint('  ${metric.name}: ${metric.average.toStringAsFixed(2)}${metric.unit}');
    }
  }
...
class PerformanceMetric {
  ...
  double get min => _values.isEmpty ? 0 : _values.reduce((a, b) => a < b ? a : b);
  double get max => _values.isEmpty ? 0 : _values.reduce((a, b) => a > b ? a : b);
