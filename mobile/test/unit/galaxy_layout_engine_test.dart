...
void main() {
  group('GalaxyLayoutEngine', () {
    ...
    group('calculateInitialLayout', () {
      ...
      test('respects existing positions when provided', () {
        ...
        // 验证现有位置被保留
        expect(positions[testNodes[0].id], equals(const Offset(100, 200)));
        expect(positions[testNodes[1].id], equals(const Offset(300, 400)));
      });
      ...
      test('groups nodes by sector', () {
        ...
        for (final entry in sectorPositions.entries) {
          if (entry.value.length > 1) {
            // 计算角度
            final angles = entry.value.map((pos) => _normalizeAngle(Offset(pos.dx, pos.dy).direction)).toList();
    ...
    group('optimizeLayoutAsync', () {
      ...
      test('optimized layout has valid positions', () async {
        ...
        for (final pos in optimizedPositions.values) {
          // 位置应该在宇宙边界内
          expect(pos.distance, lessThanOrEqualTo(GalaxyLayoutEngine.outerRadius));
  ...
  group('ViewportCuller', () {
    ...
    test('respects margin', () {
      ...
      final visibleWithMargin = cullerWithMargin.filterVisibleNodes(testNodes, positions);
      final visibleWithoutMargin = cullerWithoutMargin.filterVisibleNodes(testNodes, positions);

      expect(visibleWithMargin.length, greaterThanOrEqualTo(visibleWithoutMargin.length));
    });
