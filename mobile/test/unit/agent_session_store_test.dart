...
void main() {
  ...
  test('AgentSessionStore reuses and resets session ids', () async {
    ...
    final first = store.getOrCreateSessionId(
        AgentSessionScope.group, 'group-1', 'user-1',);
    final second = store.getOrCreateSessionId(
        AgentSessionScope.group, 'group-1', 'user-1',);

    expect(first, second);

    await store.resetSession(AgentSessionScope.group, 'group-1', 'user-1');
    final third = store.getOrCreateSessionId(
        AgentSessionScope.group, 'group-1', 'user-1',);
